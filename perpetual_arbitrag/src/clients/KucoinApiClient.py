...
class KucoinApiClient(ExchangeSpotClients, ExchangeFutureClients):
	...
	def get_futures_effective_funding_rate(self, symbol: str, seconds_before_current: int, seconds_before_estimated: int):
		"""
		...
		if self.funding_rate_enable:
			(_funding_rate, _estimated_funding_rate) = self.get_futures_funding_rate(symbol = symbol)
